...
bool isValid(string s)
{
    ...
    for (int i = 0; i < s.length(); i++)
    {
        ...
        {
            if (!valStack.empty() && ((s[i] == '}' && valStack.top() == '{') || (s[i] == ')' && valStack.top() == '(') || (s[i] == ']' && valStack.top() == '[')))
            {
                valStack.pop();
            }
            else
...
{
    cout << "111" << haystack.find(needle) << endl;
    // cout << "111" << haystack.substr(i, n) << endl;   // sustr == needle
    int j = 0;
